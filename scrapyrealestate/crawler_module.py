...
def json_to_bbdd(json_file_name, db_name, db_engine, session, Base, telegram_msg):
    ...
    for flat in data_json:
        ...
        if not bbdd_exists:
            ...
            logging.info(f'ADDING {href} TO BBDD {db_name.upper()}')
            ...
            if telegram_msg:
                if price <= data['flat_details']['max_price'] or data['flat_details'][
                    'max_price'] == 0:  # Enviar missatge a telegram si es True i el preu es <= 1000
                    try:
                        logging.info(f'SENDING {href} TO TELEGRAM GROUP')
                        # Enviem missatge tg del preu, m2, mitjana i href
                        mitja_price_m2 = '%.2f' % (price / float(m2))
                        tb.send_message(data['telegram_chatuserID'],
                                        f"{price_str} [{m2_tg}] → {mitja_price_m2}€/m²\n{href}")
                    except:
